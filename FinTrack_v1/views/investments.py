...
INVESTMENT_FILE = "data/investments.json"
...
def show_investments():
    ...
    if st.button("Add to Portfolio"):
        if not stock_name or buy_price <= 0 or units <= 0:
            ...
            data = load_json(INVESTMENT_FILE)
            data.append(new_entry)
            save_json(INVESTMENT_FILE, data)
    ...
    all_data = load_json(INVESTMENT_FILE)
    user_data = [d for d in all_data if d["username"] == st.session_state.user["username"]]
    ...
    if user_data:
        ...
        for entry in user_data:
            ...
            pnl_percentage = round((pnl / investment_cost) * 100, 2)
