...
def show_loans():
    ...
    all_loans = load_json(LOAN_FILE)
    today = date.today()

    user_loans = [l for l in all_loans if l.get("username") == st.session_state.user["username"]]
    active_loans = []
    total_emi_amount = 0
    ...
    for loan in user_loans:
        ...
        if next_due <= end:
            total_emi_amount += loan["emi_amount"]
    ...
    updated_loans = [
        ...
        if not (
            l.get("username") == st.session_state.user["username"]
            and date.fromisoformat(l["end_date"]) < today
        )
    ]
    save_json(LOAN_FILE, updated_loans)
